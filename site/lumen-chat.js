/* LUMEN Chat Widget v4 — Lumen Labs */
(function () {
  "use strict";
  ...
  var K_SID = "lumen_sid", K_HIST = "lumen_history_v4", K_AGENDA = "lumen_agenda_pending_v1";
  ...
  function uid() {
    ...
    } catch (e) { return "web-" + Date.now().toString(36) + "-" + Math.random().toString(36).slice(2, 10); }
  }
  try {
    sid = sessionStorage.getItem(K_SID) || uid(); sessionStorage.setItem(K_SID, sid);
    hist = JSON.parse(sessionStorage.getItem(K_HIST) || "[]"); if (!Array.isArray(hist)) hist = [];
    hist = hist.slice(-30); agendaPendiente = sessionStorage.getItem(K_AGENDA) === "1";
  } catch (e) { sid = uid(); hist = []; }
  ...
  var CSS = [
    ...
    '#lumen-close{margin-left:auto;width:32px;height:32px;border:0;border-radius:9px;background:transparent;color:var(--m);font-size:22px;cursor:pointer}#lumen-close:hover{background:rgba(255,255,255,.07);color:var(--t)}',
  ...
  function linkify(t){
    ...
    s=s.replace(/([\w.+-]+@[\w-]+\.[\w.-]+)/g,'<a href="mailto:$1">$1</a>'); s=s.replace(/\*\*([^*]+)\*\*/g,"<strong>$1</strong>");
    return s.replace(/\u0001(\d+)\u0001/g,function(_,i){return keep[i];});
  }
  function quick(a){chipsEl.innerHTML="";(a||[]).slice(0,4).forEach(function(t){var b=document.createElement("button");b.type="button";b.textContent=t;b.onclick=function(){chipsEl.innerHTML="";sendMsg(t==="Reintentar"&&ultimo?ultimo:t);};chipsEl.appendChild(b);});}
  ...
  function sendMsg(text){
    text=(text||"").trim(); if(!text||ocupado)return; ultimo=text; add("user",text); input.value=""; input.style.height="auto"; chipsEl.innerHTML="";
    if(privacy(text)){setTimeout(function(){add("assistant","No conozco tus datos personales por visitar la web. Uso lo que tú escribes, un identificador técnico de sesión y la página desde la que consultas. Si quieres agendar, te pediré nombre y correo antes de crear la reunión. No voy a asumir ni inventar esos datos.");quick(["Entendido","Agendar diagnóstico"]);},120);return;}
    if(yes(text)&&agendaCtx()&&!hasEmail()){setAgenda(true);setTimeout(function(){add("assistant","Perfecto. Antes de crear la invitación necesito tu nombre y correo. Puedes escribirlos así: Nombre Apellido — correo@empresa.cl. No necesito más datos para agendar.");},120);return;}

    var backend=text;
    if(agendaPendiente&&/[\w.+-]+@[\w-]+\.[\w.-]+/.test(text)){backend="El usuario aceptó el último horario propuesto. Datos que entrega ahora: "+text+". Verifica disponibilidad y agenda solo si cuentas con los datos mínimos necesarios; no inventes información.";setAgenda(false);}
    busy(true); var ctrl=typeof AbortController!=="undefined"?new AbortController():null; var timer=setTimeout(function(){if(ctrl)ctrl.abort();},45000); var tz="";try{tz=Intl.DateTimeFormat().resolvedOptions().timeZone||"";}catch(e){}
    fetch(CFG.webhook,{method:"POST",headers:{"Content-Type":"application/json","Accept":"application/json"},credentials:"omit",body:JSON.stringify({sessionId:sid,message:backend,channel:"web",page:location.pathname+location.search,pageTitle:document.title,locale:navigator.language||"es-CL",timezone:tz}),signal:ctrl?ctrl.signal:undefined})
      .then(function(r){if(!r.ok)throw new Error("HTTP "+r.status);return r.json();})
      .then(function(data){var o=parse(data);if(!o.text)throw new Error("respuesta vacía");if(/se me trab[oó]|coordinemos por whatsapp|sistema se trab[oó]/i.test(o.text)){add("error","No pude completar esa acción. Si estábamos agendando, la reunión no queda confirmada hasta que tengas un horario aceptado y hayas compartido al menos tu nombre y correo. Puedes reintentar o escribirnos a contacto@lumenhash.cl.");quick(["Reintentar","Agendar diagnóstico"]);}else{add("assistant",o.text);if(Array.isArray(o.suggestions)&&o.suggestions.length)quick(o.suggestions);}})
      .catch(function(e){add("error",e&&e.name==="AbortError"?"La respuesta está demorando más de lo esperado. No se creó ni confirmó ninguna reunión. Puedes reintentar.":"No pude conectar con el asistente. No se creó ni confirmó ninguna reunión. Puedes reintentar o escribirnos a contacto@lumenhash.cl.");quick(["Reintentar","Agendar diagnóstico"]);})
      .then(function(){clearTimeout(timer);busy(false);});
  }

  function open(){if(abierto)return;abierto=true;root.classList.add("abierto");dot.style.display="none";requestAnimationFrame(function(){root.classList.add("visible");});if(!arrancado){arrancado=true;setTimeout(function(){if(hist.length){hist.slice().forEach(function(m){add(m.role,m.text,false);});}else{add("assistant",CFG.saludo);note("LUMEN no necesita tus datos personales para orientarte. Solo te los pedirá si quieres que te contactemos o agendar.");quick(QUICK);}},180);}setTimeout(function(){input.focus();},240);}
  ...
  fab.onclick=open; close.onclick=shut; form.onsubmit=function(e){e.preventDefault();sendMsg(input.value);}; input.onkeydown=function(e){if(e.key==="Enter"&&!e.shiftKey){e.preventDefault();sendMsg(input.value);}}; input.oninput=function(){input.style.height="auto";input.style.height=Math.min(input.scrollHeight,112)+"px";}; document.addEventListener("keydown",function(e){if(e.key==="Escape"&&abierto)shut();}); if(CFG.autoabrir>0)setTimeout(open,CFG.autoabrir*1000);
  window.LumenChat={abrir:open,cerrar:shut,enviar:sendMsg,sessionId:sid};
})();
