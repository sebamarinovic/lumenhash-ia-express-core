...
(function () {
  ...
  function linkear(t) {
    ...
    // 1) Links en markdown [texto](url) — el modelo a veces los escribe así.
    s = s.replace(/\[([^\]]+)\]\((https?:\/\/[^\s)]+)\)/g, function (_, txt, url) {
      return guardar('<a href="' + url + '" target="_blank" rel="noopener noreferrer">' + txt + "</a>");
    });
    // 2) URLs sueltas (sin tocar las ya convertidas).
    s = s.replace(/(https?:\/\/[^\s<)]+)/g, function (url) {
      return guardar('<a href="' + url + '" target="_blank" rel="noopener noreferrer">' + url + "</a>");
    });
    // 3) Correos.
    ...
    return s.replace(/\u0001(\d+)\u0001/g, function (_, i) { return guardados[i]; });
  }
